...
现有 $n$ 个变量 $x_1, \cdots, x_n$ 等的赋值，其中 $x_i$ 在集合 $D_i$ 中取值。

欲通过给变量赋值，使之满足与这些变量有关的一组约束。

- 问题分为离散变量（有限 / 无穷集）、连续变量等情况。
- 约束分为一元约束（指只含一个变量）、二元约束、多元约束等。

#### 算法 1：回溯搜索 (backtracking search)
...
- (2) $x_1 \land x_5 \xRightarrow{C_5} x_6$，用 `@2` 标记其赋值层级；$x_5 \xRightarrow{C_3} x_7$，用 `@2` 标记其赋值层级。
...
- 在本例中可以选择 $\neg x_1 \lor \neg x_5$。
- 不选择 $\neg x_1 \lor \neg x_2$ 的原因是，其无法提供更多信息——回溯后这一组合就已经不会再考虑了。
